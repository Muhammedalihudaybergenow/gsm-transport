...
pduParser.generate = function (message) {
  ...
  var receiverSize = (
    '00' + parseInt(message.receiver.length, 10).toString(16)
  ).slice(-2);
  ...
  for (var i = 0; i < parts; i++) {
    ...
    if (message.encoding === '16bit') {
      ...
      if (parts === 1) var length = 70;
      else var length = 66;
    } else if (message.encoding === '7bit') {
      /* If there are more than one messages to be sent, we are going to have to put some UDH. Then, we would have space only
       * for 153 ASCII characters instead of 160 */
      if (parts === 1) var length = 160;
      else var length = 153;
    }
    ...
    if (message.encoding === '16bit') {
      ...
      if (parts > 1) size += 6; //6 is the number of data headers we append.
    } else if (message.encoding === '7bit') {
...
pduParser.parse = function (pdu) {
  ...
  var smsDeliverBits = ('00000000' + parseInt(smsDeliver).toString(2)).slice(
    -8,
  );
  var udhi = smsDeliverBits.slice(1, 2) === '1';

  var senderSize = buffer[1];
  if (senderSize % 2 === 1) senderSize++;
  ...
  if (udhi) {
    //User-Data-Header-Indicator: means there's some User-Data-Header.
    ...
    if (iei == '08') {
      //Concatenaded sms with a two-bytes reference number
      var headerLength = pdu.slice(cursor + 4, cursor + 6);
    ...
    if (encoding === '16bit')
      if (iei == '00') cursor += (udhLength - 2) * 4;
      else if (iei == '08') cursor += (udhLength - 2) * 4 + 2;
      else cursor += (udhLength - 2) * 2;
  }
  ...
  else if (encoding === '8bit') var text = ''; //TODO
